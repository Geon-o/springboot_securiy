...
## 내용
- JSON 객체로 안전하게 데이터 전송 방식
- 해당 데이터는 디지털 서명이 되어있어 신뢰할 수 있다.
- JWT는 HMAC과 RSA 알고리즘을 사용
- 서명된 토큰에 중점
  - 정보의 무결성 확인
...
- 구조
  ...
  - 페이로드(정보)
    - 클레임 존재
      1. 등록된 클레임
         - 필수는 아니지만 권장되는 클레임의 집합
      2. 개인 클레임
         - 프라이머리 키
         - 커스텀
      3. 공개 소유권 주장
  - 시그니처(서명)
    - (헤더 + 페이로드) + 서버만 알고있는 키값 -> HS256으로 암호화
  

  - HS256
    - 암호화보단 인증목적
    - HMAC 방식으로 SHA256으로 암호화(해쉬)
    - HMAC
      - 시크릿 키를 포함한 암호화방식
 ...
 - RSA
   - 공개키와 개인키
   - 암호화 방식
     - header + payload를 서버의 개인키로 lock
     - 클라리언트로부터 전달받고 검증 시 서버의 공개키로 unlock


- 통신방식
  - 클라이언트에서 localstorage에 서버로부터 받은 JWT를 저장
  - 클라이언트에서 서버로 특정 데이터를 요청시 localstorage에 저장된 JWT를 서버로 보냄
  - 서버에서는 클라이언트로부터 받은 JWT를 검증(인증)
  - 클라이언트로 넘어온 JWT의 정보 중 서버는 header와 payload와 서버의 시크릿 키를 HS256으로 암호화
  - 서버에서 암호화한 값이 JWT의 값과 같다면 인증
  - 인증 후 원하는 데이터는 payload에서 추출
